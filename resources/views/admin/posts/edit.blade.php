...
<aside id="default-sidebar" class="fixed mb-4 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0 px-6 py-8" aria-label="Sidebar">
    ...
    <h1 class="text-center font-bold text-xl mb-4">Edit Post: {{$post->title}}</h1>
       <main class="max-w-lg mx-auto bg-blue-300 p-6 rounded-xl border border-blue" >
       
        <form method="POST" action="/admin/posts/{{$post->id}}" class="mt-5" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
        <div class="mb-6">    
            ...
            <div class="mb-6">    
                ...
                <label class="block mb-2 uppercase font-bold text-xs text-gray-700"
                    for="thumbnail"
                >
                    Old thumbnail of this Post
                    
                    <img class="  w-20 h-20 rounded-full" src="/storage/{{$post->thumbnail}}" alt="Jese image">
